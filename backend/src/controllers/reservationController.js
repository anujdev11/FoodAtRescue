...
const getAllReservation = async (req, res) => {
    try {
        ...
        //console.log(foods)
        return res.status(200).json({ message: "Reservation details retrieved Successfully", success: true, data: reservations });
        //return foods;
    } catch (error) {
        return res.status(500).json({ error: error.message, message: "Unable to get all Reservation Details!!", success: false });
    }
...
const deleteReservation = async (req, res) => {
    try {
        ...
        if (count == 0) {
            return res.status(404).json({ message: "Reservation Details not found!!", success: false });
        }

        await dynamoClient.delete(params).promise().then(() => {
            return res.status(200).json({ message: "Reservation Details Deleted Successfully", success: true });
        });
    } catch (error) {
...
const updateReservation = async (req, res) => {
    try {
        ...
        await dynamoClient.put(param).promise().then(() => {
            return res.status(201).json({ message: "Reservation Details Updated Successfully", success: true });
        });
    } catch (error) {
